...
this.countRemoveWrapper(unique_id);},restoreItemIfFading:function(e,number){window.clearTimeout(Gritter['_int_id_'+number]);$(e).stop().css({opacity:1});},setFadeTimer:function(item,number){var timer_str=(this._custom_timer)?this._custom_timer:this.timer_stay;Gritter['_int_id_'+number]=window.setTimeout(function(){Gritter.fade(item,number);},timer_str);},stop:function(params){var before_close=(typeof params.before_close=='function')?params.before_close:function(){};var after_close=(typeof params.after_close=='function')?params.after_close:function(){};var wrap=$('#gritter-notice-wrapper');before_close(wrap);wrap.fadeOut(function(){$(this).remove();after_close();});},str_replace:function(search,replace,subject,count){var i=0,j=0,temp='',repl='',sl=0,fl=0,f=[].concat(search),r=[].concat(replace),s=subject,ra=r instanceof Array,sa=s instanceof Array;s=[].concat(s);if(count){this.window[count]=0;}
...
$.gritter={};$.gritter.add=function(params){try{if(!params.title||!params.text){throw'You need to fill out the first 2 params: "title" and "text"';}}catch(e){alert('Gritter Error: '+e);}
var id=Gritter.add(params);return id;}
$.gritter.remove=function(id,params){Gritter.removeSpecific(id,params||'');}
$.gritter.removeAll=function(params){Gritter.stop(params||'');}})(jQuery);